...
function solution(lines) {
	...
	*/
    ...
    for (let string of lines){
    	let timeArr = string.split(" "); //2016-09-15   20:59:57.421   0.351s
    	let hmsTimeArr = timeArr[1].split(":").map(Number);  //20 : 59 : 57.421
    	
    	let addedSecond = (hmsTimeArr[0] * 60 * 60) + (hmsTimeArr[1] * 60) + hmsTimeArr[2]; // 75597.421
    	let beforeResponse= addedSecond - ( Number(timeArr[2].replace("s",""))  )  ; // 0.351 - 0.001
    	changedSecondArr.push([beforeResponse,addedSecond]);
    }
    console.log(changedSecondArr);
    
    let cnt = 0;
    
    for (let arr of changedSecondArr){
    	
    }
    */
    var seconds = new Date().getTime() //1000wqqssfffggxx
    return answer;
}
let a =  [
